...
const CLOUDINARY_UPLOAD_PRESET = process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET || 'category_images'
      ...
      >
        {localAttributes.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '32px 0', border: '2px dashed #E0E0E0', borderRadius: 4 }}>
            <Tag style={{ width: 28, height: 28, color: '#ccc', margin: '0 auto 8px', display: 'block' }} />
            <p style={{ fontSize: 13, color: '#888', margin: '0 0 12px' }}>No attributes yet</p>
            <button type="button" onClick={addAttribute} style={{ backgroundColor: '#2874F0', color: '#fff', border: 'none', borderRadius: 4, padding: '8px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
              + Add First Attribute
            </button>
          </div>
        ) : (
          <>
              ...
              {localAttributes.map((attr, idx) => (
                <div key={attr.id} style={{ border: '1px solid #E0E0E0', borderRadius: 4, padding: 14, backgroundColor: '#FAFAFA' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    ...
                    <button type="button" onClick={() => removeAttribute(idx)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#FF6161', display: 'flex', alignItems: 'center', gap: 4, fontSize: 12 }}>
                      <Trash2 className="h-3.5 w-3.5" /> Remove
                    </button>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <div>
                      <FKLabel>Attribute Name</FKLabel>
                      <input placeholder="e.g. Brand, Size, Color" value={attr.name} onChange={e => updateAttribute(idx, 'name', e.target.value)} style={{ ...inputStyle, marginTop: 6 }} />
                    </div>
                    <div>
                      <FKLabel>Type</FKLabel>
                      <select value={attr.type} onChange={e => updateAttribute(idx, 'type', e.target.value)} style={{ ...inputStyle, marginTop: 6 }}>
                        <option value="text">Text</option>
                        <option value="number">Number</option>
                        <option value="boolean">Boolean (Yes/No)</option>
                        <option value="select">Single Select</option>
                  ...
                  {(attr.type === 'select' || attr.type === 'multiselect') && (
                    <div style={{ marginTop: 12 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
                        {attr.options?.map((opt: string, oi: number) => (
                          <div key={oi} style={{ display: 'flex', gap: 8 }}>
                            <input placeholder={`Option ${oi + 1}`} value={opt} onChange={e => updateOption(idx, oi, e.target.value)} style={{ ...inputStyle, flex: 1 }} />
                            <button type="button" onClick={() => removeOption(idx, oi)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#FF6161' }}>
                              <Trash2 className="h-4 w-4" />
                            </button>
                          </div>
            ...
            <button type="button" onClick={addAttribute} style={{ marginTop: 12, width: '100%', backgroundColor: '#EEF3FF', color: '#2874F0', border: '1px dashed #2874F0', borderRadius: 4, padding: '10px 0', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              <Plus className="h-4 w-4" /> Add Another Attribute
            </button>
          </>
        )}
...
export default function AddCategoryPage() {
  ...
  //     formData.append('public_id', `category-${Date.now()}`)
  ...
  //         thumbnail: response.data.secure_url.replace('/upload/', '/upload/w_100,h_100,c_fill,q_auto/'),
  //         publicId: response.data.public_id
  //       }
  //       setUploadedImage(imageData)
  //       setValue('iconUrl', response.data.secure_url)
  ...
  const handleImageUploadViaBackend = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file) return
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/api/v1/categories/upload/image`,
        formData,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
      ...
      if (response.data.success) {
        ...
        setValue('iconUrl', imageData.url)
        toast.success('Image uploaded successfully!')
      }
    } catch (error: any) {
      console.error('Upload error:', error)
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        if (aiData.mainCategory) {
          ...
          if (aiData.mainCategory.metaTitle) setValue("metaTitle", aiData.mainCategory.metaTitle)
          if (aiData.mainCategory.metaDescription) setValue("metaDescription", aiData.mainCategory.metaDescription)
        }

        if (currentLevel >= 1 && currentLevel <= 2 && aiData.attributes?.length) {
          console.log("Setting AI attributes:", aiData.attributes.length)
          setValue(
            "attributes",
            aiData.attributes.map((attr: any, i: number) => ({
              id: String(Date.now() + i),
  ...
  const onSubmit: SubmitHandler<CategoryFormValues> = async (data) => {
    ...
    try {
      const payload = {
        ...
        image: uploadedImage || { url: '', thumbnail: '' },
  ...
  return (
    <div style={{ backgroundColor: '#F1F3F6', minHeight: '100vh', fontFamily: "'Roboto', sans-serif" }}>
      ...
      <input
        ...
        onChange={handleImageUploadViaBackend}
        style={{ display: 'none' }}
      />
          ...
          <div>
            ...
            {activeTab === 'basic' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    {levelOptions.map(level => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p style={{ fontSize: 12, fontWeight: 700, color: isSelected ? level.color : '#333', margin: 0, lineHeight: 1.2 }}>{level.name}</p>
                          <p style={{ fontSize: 10, color: '#888', margin: 0, lineHeight: 1.3 }}>{level.description}</p>
                          <p style={{ fontSize: 10, color: '#aaa', margin: 0, fontStyle: 'italic' }}>{level.example}</p>
                        </button>
                      )
                    })}
                  </div>
                ...
                <FKCard title="Basic Information" subtitle="Core details for the category">
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    <div>
                      <FKLabel required>Category Name</FKLabel>
                      <div style={{ position: 'relative', marginTop: 6 }}>
                        <input placeholder="e.g. Electronics, Furniture" {...register('name')} style={inputStyle} />
                        <button
                          type="button"
                          onClick={generateAISuggestions}
                          disabled={isGeneratingAI}
                    ...
                    <div>
                      <FKLabel>Display Order</FKLabel>
                      <input type="number" {...register('displayOrder', { valueAsNumber: true })} style={{ ...inputStyle, marginTop: 6 }} />
                      <p style={{ fontSize: 11, color: '#888', marginTop: 4 }}>Lower numbers appear first</p>
                    </div>
                  </div>
                  <div style={{ marginTop: 16 }}>
                ...
                {/* Image Upload Card */}
                <FKCard title="Category Image" subtitle="Upload a representative image for this category">
                  <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                    <div
                    ...
                    >
                      {isUploadingImage ? (
                        ...
                        <>
                          <img src={uploadedImage.url} alt="Category" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          <button
                            type="button"
                            onClick={(e) => { e.stopPropagation(); handleRemoveImage() }}
                            style={{ position: 'absolute', top: 8, right: 8, backgroundColor: 'rgba(255,255,255,0.9)', border: 'none', borderRadius: 4, padding: 4, cursor: 'pointer', display: 'flex' }}
                          >
                        ...
                        <div style={{ textAlign: 'center', padding: 20 }}>
                          ...
                          <p style={{ fontSize: 12, color: '#888', margin: 0 }}>Click to upload</p>
                          <p style={{ fontSize: 10, color: '#bbb', margin: '4px 0 0' }}>JPG, PNG, WebP or SVG</p>
                          <p style={{ fontSize: 10, color: '#bbb', margin: 0 }}>Max 5MB</p>
                        </div>
                      )}
                    </div>
                    <div style={{ flex: 1 }}>
                      <FKLabel>Upload Image</FKLabel>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 6 }}>
                        <button
                          type="button"
                          onClick={() => fileInputRef.current?.click()}
                          disabled={isUploadingImage}
                          style={{ backgroundColor: isUploadingImage ? '#E0E0E0' : '#2874F0', color: '#fff', border: 'none', borderRadius: 4, padding: '10px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                        >
                          {isUploadingImage ? <><Loader2 className="h-4 w-4 animate-spin" /> Uploading...</> : <><Upload className="h-4 w-4" /> Choose Image</>}
                        </button>
                        <div style={{ backgroundColor: '#F0F7FF', border: '1px solid #B3D4FF', borderRadius: 4, padding: '10px 12px' }}>
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                            <Info style={{ width: 14, height: 14, color: '#2874F0', flexShrink: 0, marginTop: 1 }} />
                            <div>
                              <p style={{ fontSize: 11, color: '#0D47A1', margin: 0, fontWeight: 600 }}>Image Guidelines</p>
                              <ul style={{ margin: '4px 0 0', paddingLeft: 16, fontSize: 11, color: '#1565C0' }}>
                                <li>Use clear, high-quality images</li>
                                <li>Square format (1:1 ratio) works best</li>
                                <li>Minimum 200x200 pixels recommended</li>
                                <li>Show representative products of this category</li>
                              </ul>
                            </div>
                          </div>
                        </div>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                        ...
                        <button
                          ...
                          style={{ backgroundColor: '#EEF3FF', color: '#2874F0', border: '1px solid #B3D4FF', borderRadius: 4, padding: '8px 12px', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                          title="Random emoji"
                        >
                      ...
                      <p style={{ fontSize: 11, color: '#888', marginTop: 4 }}>
                        Tip: Use the AI icon generator for professional-looking icons
                      </p>
                      <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                        <button
                          type="button"
              ...
              <FKCard title="SEO & Metadata" subtitle="Help search engines understand your category better">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  ...
                  <div style={{ backgroundColor: '#F8F9FA', borderRadius: 4, border: '1px solid #E0E0E0', padding: 16, marginTop: 4 }}>
                    ...
                    <p style={{ fontSize: 12, color: '#006621', margin: '0 0 4px' }}>
                      https://yourstore.com/category/{(formValues.name || 'category').toLowerCase().replace(/\s+/g, '-')}
                    </p>
                    <p style={{ fontSize: 13, color: '#545454', margin: 0, lineHeight: 1.5 }}>
                      {formValues.metaDescription || 'Add a meta description to see how your category will appear in search results.'}
                    </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ backgroundColor: '#fff', borderRadius: 4, border: '1px solid #E0E0E0' }}>
              ...
              <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {isActive ? <Eye style={{ width: 15, height: 15, color: '#26A541' }} /> : <EyeOff style={{ width: 15, height: 15, color: '#FF6161' }} />}
                    <div>
                      <p style={{ fontSize: 12, fontWeight: 600, margin: 0, color: '#212121' }}>Active</p>
                      <p style={{ fontSize: 11, color: '#888', margin: 0 }}>{isActive ? 'Visible to customers' : 'Hidden from store'}</p>
                    </div>
