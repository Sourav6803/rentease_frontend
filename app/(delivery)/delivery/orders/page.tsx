...
//   const pc = PRIORITY_CONFIG[delivery.priority];
...
//   const isOverdue = delivery.schedule.deadline && Date.now() > new Date(delivery.schedule.deadline).getTime();
...
//     };
//     return map[delivery.status] ?? { label: delivery.status, action: '', color: 'from-gray-400 to-gray-500' };
//   }, [delivery.status]);
...
//     const addr = `${delivery.address.addressLine1}, ${delivery.address.city}, ${delivery.address.state} ${delivery.address.pincode}`;
...
//                         onClick={() => {
//                           setMenuOpen(false);
//                           if (item.action === 'copyAddress') { handleCopyAddress(); return; }
...
//               {delivery.contact.name.charAt(0)}
//             </div>
//             <div className="min-w-0">
//               <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{delivery.contact.name}</p>
//               <p className="text-[10px] text-gray-400">{delivery.contact.phone}</p>
//             </div>
//           </div>
...
//               <span className="font-semibold">{delivery.schedule.scheduledSlot.start}–{delivery.schedule.scheduledSlot.end}</span>
//             </div>
//           )}
...
//               )}>
//                 {delivery.charges.paymentMethod === 'cash' ? '💵 CASH' : delivery.charges.paymentMethod === 'prepaid' ? '✅ PAID' : delivery.charges.paymentMethod.toUpperCase()}
//               </span>
//             )}
//           </div>
...
//         {delivery.items.some(i => i.images?.length) && (
...
//               <p className="text-[11px] text-gray-600 dark:text-gray-400 truncate font-medium">{delivery.items[0].name}</p>
...
//                 <a
//                   href={`tel:${delivery.contact.phone}`}
...
//                   aria-label={`Call ${delivery.contact.name}`}
//                 >
