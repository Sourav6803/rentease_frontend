...
export default function RentalDetailsPage() {
  ...
  // Admins can inspect any user's rental (backend allows admin role bypass);
  // user-only actions are hidden below when viewing as admin.
  const isAdmin = session?.user?.role === 'admin' || session?.user?.role === 'super_admin'

  useEffect(() => {
    if (status === 'unauthenticated') router.push(`/login?callbackUrl=/dashboard/rentals/${rentalId}`)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <Card className="border-slate-200">
              ...
              {rental.payment.dueAmount > 0 && !isAdmin && (
                <CardFooter>
