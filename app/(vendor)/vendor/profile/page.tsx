...
import {
  ...
  Bell, Truck, CreditCard, RefreshCw,
...
function StatCard({
  ...
  return (
    <motion.div
      ...
      className="relative bg-white rounded-2xl border border-slate-100/80 p-4 shadow-sm hover:shadow-md transition-shadow overflow-hidden group"
    >
      <div
        className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
        style={{ background: `radial-gradient(circle at 90% 10%, ${color}12 0%, transparent 55%)` }}
      />
      ...
      <p className="relative text-xl font-black text-slate-900 tracking-tight leading-none">{value}</p>
      <p className="relative text-[10px] font-bold text-slate-400 mt-1.5 uppercase tracking-widest">{title}</p>
      {sub && <p className="relative text-[10px] text-slate-400 mt-0.5">{sub}</p>}
    </motion.div>
  )
}
...
function VerificationBadge({ status }: { status: string }) {
  const map = {
    ...
    rejected: { bg: 'bg-rose-50',    text: 'text-rose-700',    border: 'border-rose-200',    icon: XCircle,    label: 'Verification Failed' },
  }
  const cfg = map[status as keyof typeof map] || map.pending
  const Icon = cfg.icon
  return (
    <span className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border", cfg.bg, cfg.text, cfg.border)}>
...
function DocCard({
  ...
  return (
    <div className="border border-slate-200/80 rounded-2xl p-4 hover:border-blue-300/60 hover:bg-blue-50/30 transition-all bg-white">
      ...
      {doc ? (
        ...
        <button
          onClick={() => fileRef.current?.click()}
          className="w-full border-2 border-dashed border-slate-200 rounded-xl p-5 text-center hover:border-blue-400 hover:bg-blue-50/40 transition-all group"
        >
...
function RentalPromoBanner() {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-[#1e40af] via-[#2874f0] to-[#0ea5e9]" />
      ...
      <div className="relative p-6 md:p-7">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[11px] font-bold text-amber-300 uppercase tracking-widest">New Feature</span>
        </div>
        <h2 className="text-2xl md:text-[1.7rem] font-black text-white leading-tight mb-2">
          List Products.<br />Start Earning Today.
        </h2>
        <p className="text-blue-100 text-sm mb-5 max-w-md leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 mb-5">
          {perks.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/15">
              <Icon className="h-4.5 w-4.5 text-white mb-1.5" />
              <p className="text-white font-bold text-sm leading-tight">{title}</p>
              <p className="text-blue-100/90 text-[11px] mt-0.5 leading-snug">{desc}</p>
            </div>
          ))}
        </div>
        <div className="flex flex-wrap gap-2.5">
          <a href="/vendor/products/add" className="inline-flex items-center gap-2 px-4 py-2.5 bg-white text-blue-700 font-bold rounded-xl text-sm hover:bg-blue-50 transition-colors shadow-lg shadow-blue-900/20">
            <Package className="h-4 w-4" />
            Add Your First Product
            <ChevronRight className="h-4 w-4" />
          </a>
          <a href="/vendor/how-it-works" className="inline-flex items-center gap-2 px-4 py-2.5 bg-white/10 text-white font-bold rounded-xl text-sm hover:bg-white/20 transition-colors border border-white/25">
            How It Works
          </a>
        </div>
      </div>
...
function CategorySpotlight() {
  const cats = [
    { emoji: '🏗️', name: 'Construction Tools', demand: 'High', earning: '₹2k–8k/mo' },
    { emoji: '📸', name: 'Camera & Lenses', demand: 'Very High', earning: '₹3k–15k/mo' },
    { emoji: '🎵', name: 'Musical Instruments', demand: 'Medium', earning: '₹1.5k–6k/mo' },
    { emoji: '🏕️', name: 'Camping Gear', demand: 'Seasonal', earning: '₹800–4k/mo' },
    { emoji: '🖥️', name: 'Electronics', demand: 'High', earning: '₹2.5k–12k/mo' },
    { emoji: '👗', name: 'Designer Wear', demand: 'High', earning: '₹1k–10k/mo' },
  ]
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-black text-slate-900 text-base">Top Rental Categories</h3>
          <p className="text-xs text-slate-500 mt-0.5">Highest demand in your area right now</p>
        </div>
        <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full">Live Demand</span>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5">
        {cats.map(c => (
          <motion.div
            ...
            className="border border-slate-100 rounded-xl p-3 hover:border-blue-200 hover:bg-blue-50/40 transition-all cursor-pointer"
          >
            ...
            <div className="flex items-center justify-between mt-2 gap-1">
              ...
              <span className="text-[10px] text-slate-500 font-medium truncate">{c.earning}</span>
            </div>
          </motion.div>
        ))}
      </div>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isCurrent ? "shadow-md" : "border-slate-100 opacity-70"
              )}
              style={{ borderColor: isCurrent ? plan.color : undefined }}
            >
              {isCurrent && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full text-white mb-2 inline-block" style={{ backgroundColor: plan.color }}>
                  Current
                </span>
              )}
              <p className="font-black text-slate-900 text-sm">{plan.label}</p>
...
export default function VendorProfilePage() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      // The profile half now comes from the shared cache (deduped with the
      // layout, sidebar and header); only /vendor/stats is a direct call.
      // Both requests still start together.
      const statsPromise = fetch(`${BASE_URL}/api/v1/vendor/stats`, { headers })
      // The shared cache holds the raw backend document; this page owns a
      // stricter local `VendorProfile` describing the same document and merges
      // it with /vendor/stats below.
      const profileData = (await queryClient
        .fetchQuery(vendorProfileQueryOptions)
        .catch(() => null)) as unknown as VendorProfile | null
      const statsData = await (await statsPromise).json()

      if (profileData) {
        const nextProfile = {
          ...profileData,
          ...(statsData.success && statsData.data?.stats
            ? { stats: statsData.data.stats }
            : {}),
        }
        setProfile(nextProfile)
        setEditForm(nextProfile)
        setAvatarPreview(nextProfile?.user?.profile?.avatar || null)
        setLogoPreview(nextProfile?.business?.logo?.url || null)
        setBannerPreview(nextProfile?.business?.bannerImage?.url || null)
      } else {
  ...
  // Every mutation below rewrites the profile document, so drop the shared cache
  // entry before reloading — otherwise the reload would read the 60s-old cached
  // copy (and the sidebar/header would keep showing stale data).
  ...
  const handleGalleryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newFiles.length !== files.length) {
      toast.error('Some files were skipped (not images or >5MB)')
    }
    ...
    Promise.all(readers).then(urls => setGalleryPreview(prev => [...prev, ...urls]))
  }

  const uploadMedia = async (field: string, file: File, endpoint: string) => {
    const formData = new FormData()
    formData.append(field, file)
    const { getSession } = await import('next-auth/react')
    const sess = await getSession()
    const res = await fetch(`${BASE_URL}/api/v1/vendor${endpoint}`, {
      method: 'POST',
  ...
  const handleRemoveGalleryImage = async (publicId: string) => {
    const { getSession } = await import('next-auth/react')
    const sess = await getSession()
    const res = await fetch(`${BASE_URL}/api/v1/vendor/profile/gallery/${encodeURIComponent(publicId)}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${sess?.user?.accessToken}` },
    })
    const data = await res.json()
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        await fetch(`${BASE_URL}/api/v1/vendor/profile/avatar`, {
          method: 'POST',
      ...
      if (logoFile) {
        await uploadMedia('logo', logoFile, '/profile/upload-logo')
      }

      // Upload banner if changed
      if (bannerFile) {
        await uploadMedia('banner', bannerFile, '/profile/upload-banner')
      }
      ...
      if (galleryFiles.length > 0) {
        ...
        const { getSession } = await import('next-auth/react')
        const sess = await getSession()
        const res = await fetch(`${BASE_URL}/api/v1/vendor/profile/upload-gallery`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${sess?.user?.accessToken}` },
          body: formData,
        })
  ...
  const toggleSetting = (key: string, nested = false) => {
    if (!isEditing) return
        ...
        return {
          ...
          settings: {
            ...
            notificationPreferences: {
              ...
              [key]: !prev.settings?.notificationPreferences?.[key]
            }
          }
        }
      }
      ...
      <div className="min-h-[70vh] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="relative w-12 h-12">
            ...
            <div className="absolute inset-0 rounded-full border-[3px] border-[#2874f0] border-t-transparent animate-spin" />
          </div>
  ...
  if (!profile) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-4">
        <div className="text-center bg-white rounded-2xl border border-slate-100 p-8 shadow-sm max-w-sm w-full">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-[#2874f0] text-white rounded-xl font-bold text-sm hover:bg-[#1a5fd4] transition-colors"
          >
            Try Again
          </button>
        </div>
  ...
  const displayAvatar = avatarPreview || profile.user?.profile?.avatar
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2.5 flex-wrap mb-1">
              <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Vendor Profile</h1>
              <VerificationBadge status={profile.verification.status} />
            </div>
            <p className="text-xs sm:text-sm text-slate-500">
              {profile.vendorId} · Updated {format(new Date(profile.updatedAt), 'dd MMM yyyy, hh:mm a')}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            {isEditing ? (
              <>
                <button
                  onClick={() => {
                    setIsEditing(false)
                    setEditForm(profile)
                    setAvatarPreview(profile.user?.profile?.avatar || null)
                    setAvatarFile(null)
                    setLogoPreview(profile.business?.logo?.url || null)
                    setLogoFile(null)
                    setBannerPreview(profile.business?.bannerImage?.url || null)
                    setBannerFile(null)
                    setGalleryFiles([])
                    setGalleryPreview([])
                  }}
                  className="px-4 py-2 border border-slate-200 rounded-xl text-slate-700 font-semibold text-sm hover:bg-slate-50 transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  disabled={isSaving}
                  className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-xl font-bold text-sm hover:bg-emerald-700 transition-colors disabled:opacity-60 shadow-sm shadow-emerald-200"
                >
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-[#2874f0] text-white rounded-xl font-bold text-sm hover:bg-[#1a5fd4] transition-colors shadow-sm shadow-blue-200"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-4 xl:col-span-3 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
              <div className="h-20 bg-gradient-to-br from-[#1e40af] via-[#2874f0] to-[#0ea5e9] relative">
                ...
                />
                {(profile.business?.bannerImage?.url || bannerPreview) ? (
                  <img
                    src={bannerPreview || profile.business?.bannerImage?.url}
                    alt="Banner"
              ...
              <div className="px-4 pb-5">
                <div className="relative -mt-10 mb-3 flex items-end justify-between">
                  <div className="relative group">
                    <div className="w-[72px] h-[72px] rounded-2xl ring-[3px] ring-white bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center shadow-md overflow-hidden">
                      {displayAvatar ? (
                        <img src={displayAvatar} alt="Avatar" className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-2xl font-black text-slate-500">
                          {profile.user.profile.firstName?.[0] || 'V'}
                        </span>
                      )}
                    </div>
                    ...
                    {isEditing && (
                      <>
                        <button
                          ...
                          className="absolute -bottom-1 -right-1 w-7 h-7 bg-[#2874f0] rounded-lg flex items-center justify-center shadow-md hover:bg-[#1a5fd4] transition-colors ring-2 ring-white"
                  ...
                  <div className={cn(
                    ...
                    profile.status.isActive ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-600"
                  )}>
                    {profile.status.isActive ? '● Active' : '● Inactive'}
                  </div>
                </div>

                {(profile.business?.logo?.url || logoPreview) && (
                  <div className="mb-2">
                    <img
                      src={logoPreview || profile.business?.logo?.url}
                      alt="Business Logo"
                ...
                {isEditing && (
                  <div className="mb-2">
                    ...
                    >
                      ...
                      {profile.business?.logo?.url ? 'Change Logo' : 'Upload Logo'}
                    </button>
                  </div>
                )}

                <h2 className="font-black text-slate-900 text-lg leading-tight">{profile.business.name}</h2>
                ...
                {isEditing && avatarFile && (
                  <p className="text-[10px] text-blue-600 font-medium mt-1.5 flex items-center gap-1">
                    <CheckCircle className="h-3 w-3" /> New photo selected — save to apply
                  </p>
                )}
                ...
                <div className="mt-4 space-y-2">
                  {[
                    ...
                    <div key={val} className="flex items-center gap-2.5 text-sm">
                      ...
                      <span className="text-slate-600 truncate text-[13px]">{val}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 gap-2 text-center">
                  <div className="bg-slate-50 rounded-xl p-2.5">
                    <p className="text-base font-black text-slate-900">{profile.products?.rented ?? 0}</p>
                    <p className="text-[10px] text-slate-500 font-semibold">Rented Out</p>
                  </div>
                  <div className="bg-slate-50 rounded-xl p-2.5">
                    <p className="text-base font-black text-slate-900">{profile.products?.available ?? 0}</p>
                    <p className="text-[10px] text-slate-500 font-semibold">Available</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="mt-4 pt-3.5 border-t border-slate-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map(s => (
                      <Star
                        ...
                        className={cn(
                          "h-3.5 w-3.5",
                          s <= Math.round(profile.performance?.rating?.average ?? 0)
                  ...
                  <span className="font-black text-slate-900 text-sm">
                    {(profile.performance?.rating?.average ?? 0).toFixed(1)}
                  </span>
                  <span className="text-slate-400 text-[10px]">({profile.performance?.rating?.count ?? 0})</span>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-8 xl:col-span-9 space-y-4">
            ...
            {/* Tabs */}
            <div className="bg-white rounded-2xl border border-slate-100 p-1.5 flex gap-1 overflow-x-auto shadow-sm">
              {TABS.map(({ id, label, icon: Icon }) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all flex-1 justify-center min-w-[100px]",
                      active
                        ? "bg-[#2874f0] text-white shadow-sm shadow-blue-200"
            ...
            <AnimatePresence mode="wait">
                ...
                <motion.div key="overview" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Cities</p>
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {profile.addresses.serviceableCities?.length > 0
                            ? profile.addresses.serviceableCities.map(c => (
                              <span key={c._id} className={cn(
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Pincodes</p>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                    <h3 className="font-black text-slate-900 mb-1 text-sm">How Renting Out Works</h3>
                    <p className="text-xs text-slate-500 mb-4">Get started in 4 simple steps</p>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {[
                        { step: '01', icon: Package, title: 'List Your Items', desc: 'Add photos, description & set your rental price', color: '#2874f0' },
                        { step: '02', icon: Bell, title: 'Receive Requests', desc: 'Get notified and confirm bookings in one click', color: '#0891b2' },
                        { step: '03', icon: Truck, title: 'Deliver or Hand Over', desc: 'Ship or hand-deliver. Mark as dispatched', color: '#16a34a' },
                        { step: '04', icon: Wallet, title: 'Get Paid', desc: 'Payment released after successful rental', color: '#8b5cf6' },
                      ].map(s => (
                        <div key={s.step} className="relative">
                          <div className="w-8 h-8 rounded-xl flex items-center justify-center mb-2.5" style={{ backgroundColor: `${s.color}15` }}>
                            <s.icon className="h-4 w-4" style={{ color: s.color }} />
                          </div>
                          <span className="absolute top-0 right-0 text-[10px] font-black text-slate-200">{s.step}</span>
              ...
              {/* Business */}
              {activeTab === 'business' && (
                <motion.div key="business" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-5">
                        ...
                        <div key={key}>
                          ...
                          {isEditing ? (
                            <input
                              ...
                              className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2874f0]/25 focus:border-[#2874f0] transition-shadow"
                            />
                          ) : (
                            <p className="text-sm font-semibold text-slate-800 py-2">{value}</p>
                          )}
                        </div>
                      ))}
                    </div>
                    ...
                    <div>
                      ...
                      {isEditing ? (
                        <textarea
                          ...
                          rows={4}
                          className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2874f0]/25 focus:border-[#2874f0] resize-none"
                          placeholder="Tell customers about your business…"
                        />
                      ) : (
                        <p className="text-sm text-slate-600 leading-relaxed">
                          {profile.business.description || (
                            <span className="italic text-slate-400">No description yet. Businesses with descriptions get 3× more inquiries.</span>
                          )}
                        </p>
                      )}
                    </div>
                    ...
                    {/* Banner Image */}
                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Banner Image</label>
                      {(profile.business?.bannerImage?.url || bannerPreview) ? (
                        <div className="mb-2 rounded-xl overflow-hidden border border-slate-200 relative group">
                          <img
                            src={bannerPreview || profile.business?.bannerImage?.url}
                            alt="Banner"
                        ...
                        <div className="mb-2 rounded-xl border-2 border-dashed border-slate-200 h-40 flex items-center justify-center bg-slate-50">
                          {isEditing ? (
                            <button
                              onClick={() => bannerInputRef.current?.click()}
                            ...
                            <span className="text-xs text-slate-400 italic">No banner image uploaded</span>
                          )}
                        </div>
                      )}
                      <input
                    ...
                    {/* Logo */}
                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Business Logo</label>
                      {(profile.business?.logo?.url || logoPreview) ? (
                        ...
                        <div className="mb-2 inline-flex items-center gap-2">
                          {isEditing ? (
                            <button
                              onClick={() => logoInputRef.current?.click()}
                              className="flex items-center gap-1.5 px-3 py-2 border-2 border-dashed border-slate-200 rounded-lg text-slate-400 hover:text-blue-600 hover:border-blue-400 transition-colors"
                            >
                    ...
                    {/* Gallery */}
                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Gallery</label>
                      <input
                        ref={galleryInputRef}
                        type="file"
                        accept="image/*"
                      ...
                      />
                      {(profile.business?.gallery?.length || galleryPreview.length) > 0 ? (
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mb-2">
                          {profile.business?.gallery?.map((img, idx) => (
                            <div key={`g-${idx}`} className="relative aspect-square rounded-xl overflow-hidden border border-slate-200 group">
                              <img src={img.url} alt={`Gallery ${idx + 1}`} className="w-full h-full object-cover" />
                              {isEditing && (
                                <button
                                  onClick={async () => {
                                    if (img.publicId) {
                                      await handleRemoveGalleryImage(img.publicId)
                                    }
                                  }}
                          ...
                          {galleryPreview.map((url, idx) => (
                            <div key={`p-${idx}`} className="relative aspect-square rounded-xl overflow-hidden border border-slate-200">
                              <img src={url} alt={`New gallery ${idx + 1}`} className="w-full h-full object-cover" />
                            </div>
                          ))}
                        </div>
                      ) : (
                        <div className="mb-2">
                          {isEditing ? (
                            ...
                            >
                              <Upload className="h-4 w-4" />
                              <span className="text-xs font-semibold">Add Gallery Images</span>
                            </button>
                          ) : (
                            <span className="text-xs text-slate-400 italic">No gallery images yet</span>
                          )}
                        </div>
                      )}
                      {isEditing && ((profile.business?.gallery?.length ?? 0) > 0 || galleryPreview.length > 0) && (
                        <div className="mt-2">
                          <button
                            onClick={() => galleryInputRef.current?.click()}
                            className="text-xs text-blue-600 font-semibold hover:underline flex items-center gap-1"
              ...
              {/* Documents */}
              {activeTab === 'documents' && (
                <motion.div key="documents" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className="space-y-4">
                    ...
                    <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-emerald-600">
                          Verified on {profile.verification.verifiedAt ? format(new Date(profile.verification.verifiedAt), 'dd MMM yyyy') : '—'}
                        </p>
                      </div>
                    </div>
                  )}
                  {profile.verification.status === 'pending' && (
                    <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-amber-600">Usually takes 1–2 business days.</p>
                      </div>
                    </div>
                  )}

                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                    <h3 className="font-black text-slate-900 mb-1 flex items-center gap-2 text-sm">
                      <Shield className="h-4 w-4 text-[#2874f0]" />
                      KYC Documents
                    </h3>
                    <p className="text-xs text-slate-500 mb-4">Required for verification and payouts</p>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                    <div className="flex items-start gap-3">
                      <Lock className="h-4.5 w-4.5 text-slate-500 shrink-0 mt-0.5" />
                      <p className="text-xs text-slate-500 leading-relaxed">
                        All documents are encrypted with bank-grade 256-bit SSL. Used only for identity verification and never shared with third parties.
                      </p>
                    </div>
                  </div>
                </motion.div>
              ...
              {/* Banking */}
              {activeTab === 'banking' && (
                <motion.div key="banking" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className="space-y-4">
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                    ...
                    {profile.bankDetails ? (
                      <div className="relative bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-5 text-white overflow-hidden">
                        ...
                        <div className="relative">
                          ...
                          <p className="text-lg font-black">{profile.bankDetails.accountHolderName}</p>
                          ...
                          {profile.bankDetails.upiId && (
                            <div className="mt-4 pt-3 border-t border-white/10">
                              <p className="text-[10px] text-slate-400">UPI ID</p>
                      ...
                      <div className="border-2 border-dashed border-slate-200 rounded-2xl p-7 text-center">
                        <CreditCard className="h-7 w-7 mx-auto text-slate-300 mb-2.5" />
                        ...
                        <button className="px-4 py-2 bg-[#2874f0] text-white rounded-xl font-bold text-sm hover:bg-[#1a5fd4] transition-colors">
                          Add Bank Account
                        </button>
                      </div>
                    )}
              ...
              {/* Settings */}
              {activeTab === 'settings' && (
                <motion.div key="settings" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className="space-y-4">
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                    <h3 className="font-black text-slate-900 mb-4 flex items-center gap-2 text-sm">
                    ...
                    <div className="mt-5 pt-4 border-t border-slate-100 space-y-1">
                      {[
                        { key: 'autoConfirmBookings', label: 'Auto-confirm Bookings', desc: 'Automatically confirm rental requests' },
                        { key: 'instantBooking', label: 'Instant Booking', desc: 'Allow customers to book without approval' },
                      ].map(({ key, label, desc }) => {
                        const enabled = isEditing
                          ? !!editForm.settings?.[key]
                          : !!(profile.settings as any)?.[key]
                        return (
                          <div key={key} className="flex items-center justify-between py-2.5">
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                enabled ? "bg-[#2874f0]" : "bg-slate-200",
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                    ...
                    <div className="space-y-1">
                      {[
                        { key: 'newRentals', label: 'New Rental Requests', desc: 'When a customer places a rental order' },
                        { key: 'cancellations', label: 'Cancellations', desc: 'When a customer cancels their booking' },
                        { key: 'payments', label: 'Payment Updates', desc: 'Payouts and payment status changes' },
                        { key: 'reviews', label: 'Customer Reviews', desc: 'When you receive a new review' },
                        { key: 'dailyDigest', label: 'Daily Summary Digest', desc: 'Daily email summary of activity' },
                      ].map(({ key, label, desc }) => {
                        const enabled = isEditing
                          ? !!editForm.settings?.notificationPreferences?.[key]
                          : !!(profile.settings?.notificationPreferences as any)?.[key]
                        return (
                    ...
                    {isEditing && (
                      <p className="text-[11px] text-slate-400 mt-3 italic">Click Save Changes to apply notification preferences.</p>
                    )}
                  </div>
                </motion.div>
              )}
