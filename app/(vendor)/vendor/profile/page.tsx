  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        setProfile(data.data.profile)
        setEditForm(data.data.profile)
        setAvatarPreview(data.data.profile?.user?.profile?.avatar || null)
        setLogoPreview(data.data.profile?.business?.logo?.url || null)
        setBannerPreview(data.data.profile?.business?.bannerImage?.url || null)
      }
    } catch {
      toast.error('Failed to load profile')
    } finally {
