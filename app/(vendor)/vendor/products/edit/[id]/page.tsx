...
//     // Find first error section and navigate to it
//     const firstErrorKey = Object.keys(validationErrors)[0]
//     if (firstErrorKey === 'basicInfo' || firstErrorKey === 'category') {
//       setActiveSection('basic')
//     } else if (firstErrorKey === 'images') {
//       setActiveSection('images')
//     } else if (firstErrorKey === 'pricing') {
//       setActiveSection('pricing')
//     } else if (firstErrorKey === 'inventory') {
//       setActiveSection('inventory')
//     } else if (firstErrorKey === 'condition') {
...
//       // Collect only new files (ones with file property)
//       const newFiles = formData.media.images.filter((img) => img.file)
...
//       // Use the SAME endpoint as the add product page
//       const uploadRes = await fetch(`${BASE_URL}/api/v1/vendor/products/upload-images`, {
//         method: 'POST',
//         headers: {
...
//         throw new Error(uploadData.message || uploadData.error?.message || 'Image upload failed')
//       }
      
//       // Handle both possible response structures (like add product page does)
//       const uploadedImagesData = uploadData.data?.images || uploadData.images || []
      
//       if (!uploadedImagesData || uploadedImagesData.length === 0) {
//         throw new Error('No images returned from server')
//       }
      
//       // Map the uploaded images to match your ProductImage interface
//       const newImages: ProductImage[] = uploadedImagesData.map((img: any) => ({
//         url: img.url,
//         thumbnail: img.thumbnail || img.url.replace('/upload/', '/upload/w_200,h_200,c_fill/'),
//         isPrimary: false,
//         // No file property needed - these are now uploaded and stored in cloud
//       }))
      
//       // Get existing images (ones without file property - already uploaded)
//       const existingImages = formData.media.images
//         .filter((img) => !img.file)
//         .map(({ url, thumbnail, isPrimary }) => ({ url, thumbnail, isPrimary }))
      
//       // Determine which image should be primary
//       const hasExistingPrimary = existingImages.some(img => img.isPrimary)
//       const hasNewPrimary = newFiles.some(img => img.isPrimary)
      
//       if (!hasExistingPrimary && !hasNewPrimary && newImages.length > 0) {
//         // No primary image selected, make the first new image primary
//         newImages[0].isPrimary = true
//       } else if (hasNewPrimary) {
//         // Find which of the new files was marked as primary and set the corresponding uploaded image
//         const primaryIndex = newFiles.findIndex(img => img.isPrimary)
//         if (primaryIndex >= 0 && primaryIndex < newImages.length) {
//           newImages[primaryIndex].isPrimary = true
//         }
//       }
...
//       }
//     } else {
//       // No new images - just keep existing ones (remove file references)
//       submitData.media = {
//         images: formData.media.images.map(({ url, thumbnail, isPrimary }) => ({
//           url,
//           thumbnail,
//           isPrimary,
//         })),
...
interface ProductFormData {
  ...
  pricing: Pricing
  inventory: Inventory
  media: { images: ProductImage[]; videos?: any[] }
  condition: string
...
function SectionCard({ title, icon: Icon, accent = false, children }: {
  ...
  return (
    ...
    >
      <div className={`px-6 py-4 border-b border-slate-100 flex items-center gap-3 ${accent ? 'bg-gradient-to-r from-[#f8fbff] to-white' : ''}`}>
        <div className="w-8 h-8 rounded-lg bg-[#2874f0]/10 flex items-center justify-center">
          <Icon className="h-[18px] w-[18px] text-[#2874f0]" />
        </div>
        <h2 className="text-base font-bold text-slate-800">{title}</h2>
      </div>
      <div className="p-6">{children}</div>
...
export default function EditProductPage() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          pricing: prod.pricing,
          inventory: prod.inventory,
          media: { images: prod.media?.images || [], videos: prod.media?.videos || [] },
          condition: prod.condition,
          category: prod.category || '',
          specifications: prod.specifications || {},
  ...
  const handleInputChange = <S extends keyof ProductFormData>(
    ...
    setErrors((prev) => {
      ...
      if (section === 'basicInfo' && newErrors.basicInfo) {
        delete (newErrors.basicInfo as any)[field]
        if (Object.keys(newErrors.basicInfo).length === 0) delete newErrors.basicInfo
      } else if (section === 'pricing' && newErrors.pricing) {
        delete (newErrors.pricing as any)[field]
        if (Object.keys(newErrors.pricing).length === 0) delete newErrors.pricing
      } else if (section === 'inventory' && newErrors.inventory) {
        delete (newErrors.inventory as any)[field]
        if (Object.keys(newErrors.inventory).length === 0) delete newErrors.inventory
      }
      return newErrors
    })
  }
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (hasNewImages) {
        ...
        // Determine which image should be primary
        const hasExistingPrimary = existingImages.some(img => img.isPrimary)
        const primaryNewFileIndex = newFiles.findIndex(img => img.isPrimary)
        
        if (!hasExistingPrimary && primaryNewFileIndex === -1 && newImages.length > 0) {
          // No primary image selected, make the first new image primary
          newImages[0].isPrimary = true
        } else if (primaryNewFileIndex >= 0 && primaryNewFileIndex < newImages.length) {
          // Set the corresponding uploaded image as primary
          newImages[primaryNewFileIndex].isPrimary = true
        }
      ...
      if (data.success) {
        ...
        // Short delay to show success state
        setTimeout(() => {
          router.push('/vendor/products/all-products')
        }, 800)
      } else {
        throw new Error(data.message || data.error?.message || 'Failed to update product')
      }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <Field label="Monthly Rent" required error={errors.pricing?.monthlyRent} hint="Set competitive pricing based on market rates">
                        <Input
                          type="number"
                          value={formData.pricing.monthlyRent}
                          onChange={(e) => handleInputChange('pricing', 'monthlyRent', parseFloat(e.target.value) || 0)}
                          placeholder="e.g., 999"
                          prefix="₹"
                          error={errors.pricing?.monthlyRent}
                        />
                      ...
                      <Field label="Security Deposit" error={errors.pricing?.securityDeposit} hint="Refundable deposit collected before rental">
                        <Input
                          type="number"
                          value={formData.pricing.securityDeposit}
                          onChange={(e) => handleInputChange('pricing', 'securityDeposit', parseFloat(e.target.value) || 0)}
                          placeholder="e.g., 5000"
                          prefix="₹"
                          error={errors.pricing?.securityDeposit}
                        />
                      ...
                      <Field label="Delivery Charges" hint="Set to 0 for free delivery">
                        <Input
                          type="number"
                          value={formData.pricing.deliveryCharges}
                          onChange={(e) => handleInputChange('pricing', 'deliveryCharges', parseFloat(e.target.value) || 0)}
                          placeholder="e.g., 199"
                          prefix="₹"
                        />
                      </Field>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <Field label="Available Quantity" hint={`Currently rented: ${Math.max(0, formData.inventory.totalQuantity - formData.inventory.availableQuantity)}`}>
                        <Input
                          type="number"
                          value={formData.inventory.availableQuantity}
                          onChange={(e) => handleInputChange('inventory', 'availableQuantity', parseInt(e.target.value) || 0)}
                          min={0}
                          max={formData.inventory.totalQuantity}
                        />
                      </Field>
